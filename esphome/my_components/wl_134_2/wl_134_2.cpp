...
static const char *const TAG = "wl_134_2.sensor";
static const uint8_t ASCII_CR = 0x0D;
static const uint8_t ASCII_NBSP = 0xFF;
static const int MAX_DATA_LENGTH_BYTES = 6;

void Wl134_2Component::setup() { this->publish_state(""); }

void Wl134_2Component::loop() {
  while (this->available() >= RFID134_PACKET_SIZE) {
    Wl134_2Component::Rfid134Error error = this->read_packet_();
...
Wl134_2Component::Rfid134Error Wl134_2Component::read_packet_() {
  ...
  if (packet[RFID134_PACKET_END_CODE] == 0x07 ){
      
      } else if (packet[RFID134_PACKET_END_CODE] == 0x03 ){
...
uint64_t Wl134_2Component::hex_lsb_ascii_to_uint64_(const uint8_t *text, uint8_t text_size) {
  uint64_t value = 0;
  uint8_t i = text_size;
  do {
    i--;
...
void Wl134_2Component::dump_config() {
